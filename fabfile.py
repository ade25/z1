...
env.local_dir = '/Users/cb/ops/z1'

env.hosts = ['zope1.kreativkombinat.de']
env.hosted_sites = {
}
...
def deploy(actor=None):
    ...
    msg = '[zope1] zope1.kreativkombinat.de server configuration deployed by %(actor)s' % opts
    user = 'fabric'
    icon = ':shipit:'
...
def update(sitename=None, actor=None):
    ...
    msg = '[zope1] %(sitename)s deployed by %(actor)s' % opts
    user = 'fabric'
    icon = ':shipit:'
    slack.chat.post_message('#development', msg, username=user, icon_emoji=icon)
